...
function toggleZoom(){
    ...
    // drawLine();
        ...
        _e = dataX.length;
        _s = 0;
        let _r = {s:0, e:dataX.length, zoomStatus:zoomStatus};
        gVar.zoom = _r;
...
for ( let i= 0; i < _i ; i++ ){
    dataX.push( Math.floor(i/_interval));
...
y = 30;
y += (( MMU[1] - d ) / MMU[2] ) * line ;
chart.circle(x, y)
dataT.push({ d : Math.round(y) + "," + Math.round(x) +","+Math.round(d) +","+f +"," + MMU + ',' + Number(_indX)})
x += xs;
},
...
y += ( ( MMU[1] - d ) / MMU[2] ) * line ;
chart.circle(x, y)
dataT.push({ d : Math.round(y) + "," + Math.round(x) +","+ Math.round(d * 10)/10 +","+f + "," + MMU + ',' + Number(_indG)})
    ...
    // for(xdata of dataX){
        ...
        // }else{
            ...
            if (_length > 24000){                             // 1h 2h 3h 4h 5h 6h
                   _factor = Math.floor(_length/3600)*10 // hours
                //    if(!zoomStatus){_factor = 1}
                    ctx.fillText(xdata + (xi)*_factor+'u', x +_factor*xs - xs ,h-10); 
                    x += xs*_factor;
              }else 
             // Show in minutes 
            if (_length > 420){                           // 1m 2m 3m 4m 5m 6m
                  _factor = Math.floor(_length/60)*10 // minutes
                //   if(!zoomStatus){_factor = 1}
                  ctx.fillText(xdata + (xi)*_factor+'m', x +_factor*xs - xs ,h-10); 
                  x += xs*_factor;
            }else 
            // Show seconds
            if (_length > 200){
                    _factor = Math.floor(_length/100)*10    // 30 60 90
                    // if(!zoomStatus){_factor = 1}
                    ctx.fillText(xdata + (xi)*_factor, x +_factor*xs - xs ,h-10); 
                    x += xs*_factor;
            }else 
            if (_length > 10){
